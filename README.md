...
Library (steganography.py): **stegano_last1bit_diff_EXP** - function to visualize difference in 2 provided pictures of the same size. The lighter the color a resulting pixel has, the more the difference there was on that pixel between the two original pictures. Useful for spotting more signifficant changes within images (spot the difference games, find out the photoshopped parts of an image).
